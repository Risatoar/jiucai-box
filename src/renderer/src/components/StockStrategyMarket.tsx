  ...
  return <section className="stock-live-market" aria-label={`${card.name}实时行情`}>
    ...
    {loading && !quote && !bars.length ? <div className="stock-live-state"><span className="kline-loader" />正在读取真实行情</div> : error && !quote && !bars.length ? <div className="stock-live-state error">{error}</div> : <>
      <div className="stock-live-quote">
        ...
        <dl><div><dt>开</dt><dd>{quote?.open == null ? '--' : priceText(quote.open)}</dd></div><div><dt>高</dt><dd>{quote?.high == null ? '--' : priceText(quote.high)}</dd></div><div><dt>低</dt><dd>{quote?.low == null ? '--' : priceText(quote.low)}</dd></div><div><dt>成交额</dt><dd>{amountText(quote?.amount ?? null)}</dd></div><div><dt>来源</dt><dd>{quote?.source || 'K 线行情'}</dd></div><div><dt>时间</dt><dd>{marketTime(quote?.exchangeTime ?? bars.at(-1)?.time ?? null)}</dd></div></dl>
      </div>
