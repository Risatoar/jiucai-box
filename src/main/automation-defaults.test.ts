...
describe('default automation templates', () => {
  it('contains full-session candidate refresh and the 12:00 midday review', async () => {
    ...
    expect(template.tasks.find((task) => task.id === 'candidate_refresh')).toMatchObject({ title: '盘中候选池刷新', description: '结合盈利目标全市场筛选0至5个合格候选，并标出买入就绪标的' })
    ...
    expect(template.tasks.find((task) => task.id === 'intraday')).toMatchObject({ description: '监控持仓强买卖信号，并扫描我的收藏和AI发现的高质量买点' })
